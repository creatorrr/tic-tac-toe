module AI ( genMoves, next', max', min', minimax )  where
...
next' :: [Move] -> [[Move]]
next' = map (\(p, g, s) -> map (wrap p s . play g) $ posLeft g) where
    wrap p s g' = (p, g', s)

genMoves :: Grid -> [Move]
genMoves game = zip3 nextPoss nextGames (repeat Nothing) where
  nextPoss = posLeft game 
  nextGames = zipWith play (repeat game) nextPoss

max' :: Depth -> [Move] -> Move
max' d moves = best . score d $ if done' moves
  then moves
  else map (min' (d+1)) $ next' moves

min' :: Depth -> [Move] -> Move
min' d moves = worst . score d $ if done' moves
  then moves
